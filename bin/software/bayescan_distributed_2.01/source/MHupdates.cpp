...
void update_ancestral_freq()
{
    ...
    for (int i=0;i<I;i++) // cycle over loci
    {
        if (!discarded_loci[i])
        {
            ...
            u=randgen.randDblExc(BwPrp)+max(0.0+epsilon,old_freq-e_ancestral[i]);
    ...
    for (int j=0;j<J;j++) //cycle over populations
    {
        ...
        for (int i=0;i<I;i++) // cycle over loci
        {
            if (!discarded_loci[i])
            {
                ...
                // propose the new values
                if (randgen.rand()<1-prop_small)
                    u=randgen.rand(BwPrp)+max(0.0+epsilon,old_freq-e_freq[i][j]);
                else
                    u=randgen.rand(BwPrp2)+max(0.0+epsilon,old_freq-e_small);
...
void update_f_random()
{
    ...
    if (prior_fis_unif)
    {
        for (int j=0;j<J;j++) // cycle over populations
        {
            ...
            // propose the new values
            u=randgen.randDblExc(BwPrp)+max(prior_fis_lb,old_f-e_f[j]);
            f[j]=u;
            // calculate backward proposition probability =q(p,p')
    ...
    else   // beta prior
    {
        for (int j=0;j<J;j++) // cycle over populations
        {
            ...
            // propose the new values
            u=randgen.rand(BwPrp)+max(0.0,old_f-e_f[j]);
...
void update_beta()
{
    ...
    for (int j=0;j<J;j++) // cycle over populations
    {
        ...
        // make the move
        e=randgen.randNorm(0,var_prop_beta[j]);
        beta[j]=beta[j]+e;
...
void update_alpha_i(int i)
{
    ...
    // make the move
    e=randgen.randNorm(0,var_prop_alpha[i]);
    alpha[i]=alpha[i]+e;
...
void update_freq_codominant()
{
    ...
    for (int i=0;i<I;i++) // cycle over loci
    {
        if (!discarded_loci[i])
        {
            ...
            m=randgen.randInt(freq_locus[i].ar-1);
            do
            {
                n=randgen.randInt(freq_locus[i].ar-1);
            }
            while (n==m);
            ...
            // propose the new values
            u=randgen.randDblExc(BwPrp)+max(0.0,old_m-e_ancestral[i]);
...
// update alpha i
///////////////////////////////////
void update_alpha_i_codominant(int i)
{
    double r; // random value to accept/reject the move
    double A; // log of A in MH algorithm : accept move with probability min(1,A)
...
void update_f_intensity() // selection2
{
    ...
    for (int j=0;j<J;j++)
    {
        ...
        // propose the new values
        u=randgen.randDblExc(BwPrp)+max(0.0,old_f-e_f[j]);
        f[j]=u;
...
//double u; // proposal random value
    ...
    for (int i = 0; i < I; i++)
    {
        if (!discarded_loci[i])
        {
            ...
            // make the move
            e=randgen.randNorm(0,var_prop_sigma1[i]);
            ...
            // store the old logikelihood and calculate the new logikelihood
            old_log_likelihood=log_likelihood;
//	log_likelihood=allelecount_logikelihood();
...
//double u; // proposal random value
    ...
    for (int i = 0; i < I; i++)
    {
        if (!discarded_loci[i])
        {
            ...
            // make the move
            e=randgen.randNorm(0,var_prop_sigma2[i]);
