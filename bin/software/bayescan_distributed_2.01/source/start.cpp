...
int main(int argc, char **argv)
{
	...
	opt->addUsage( " --------------------------- " );
	opt->addUsage( " | BayeScan 2.0 usage:     | " );
	opt->addUsage( " --------------------------- " );
...
// write mean fst values
    if (!opt->getFlag( "fstat" ))
    {
        fst_file << "    " << "prob" << "  log10(PO)" /*<< "   pval"*/ << "  alpha" << "  fst" << endl;
	}
    ...
    if (cur_out>0)
    {
        ...
        for (int i=0;i<I;i++)
        {
            if (!discarded_loci[i])
            {
                ...
                if (!opt->getFlag( "fstat" ))
                    fst_file << (i+1) << "  " << p_rj << " " << bf << " " <<  setw(5) << setprecision(5) << setiosflags(ios::showpoint) << post_alpha[i]/(double)cur_out/*nb_alpha[i]*/ << " " <<  post_fst[i]/(double)cur_out/*nb_alpha[i]*/ << endl;
            }
        }
    }
